...
import io.labs.dotanuki.magicmodules.internal.model.MagicModulesError.CantExtractGradleCoordinates
...
internal object ExtractCoordinates {
    ...
    fun moduleName(rootProjectName: String, buildScript: GradleBuildScript): CanonicalModuleName =
        extractSegments(rootProjectName, buildScript)
    ...
    private fun extractSegments(rootProjectName: String, buildScript: GradleBuildScript): List<String> =
        with(buildScript) {
            ...
            if (splittedPath.size != 2) throw CantExtractGradleCoordinates

            val segments = splittedPath[1].split("/").drop(1).dropLast(1)

            when (segments.size) {
                0 -> throw CantExtractGradleCoordinates
                else -> segments
            }
        }
